...
if (isset($_GET['hapus'])) {
    ...
    $cek = $koneksi->prepare("SELECT id_booking FROM tb_booking WHERE id_booking = :id AND id_user = :uid");
    $cek->execute([':id' => $idHapus, ':uid' => $id_user]);

    if ($cek->fetch()) {
        $del = $koneksi->prepare("DELETE FROM tb_booking WHERE id_booking = :id");
        $del->execute([':id' => $idHapus]);
        header('Location: index.php?sukses=Booking%20berhasil%20dihapus');
        exit;
    } else {
        header('Location: index.php?gagal=Booking%20tidak%20ditemukan%20atau%20bukan%20milik%20Anda');
        exit;
    }
}
    ...
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table align-middle mb-0">
                <thead><tr><th>Plat</th><th>Area</th><th>Tanggal</th><th>Jam</th><th>Status</th><th></th></tr></thead>
                <tbody>
                <?php foreach ($bookingTerbaru as $b): ?>
                    <tr id="row-<?= $b['id_booking'] ?>">
                        ...
                        <td>
                            ...
                            $badge = $labelBadge[$b['status']] ?? 'bg-secondary';
                            $labelStatus = $b['status'] !== '' ? ucfirst($b['status']) : 'Tidak diketahui';
                            ?>
                            <span class="badge <?= $badge ?>"><?= htmlspecialchars($labelStatus) ?></span>
                        </td>
                        <td class="text-end">
                            <a href="index.php?hapus=<?= $b['id_booking'] ?>"
                ...
                <?php if (empty($bookingTerbaru)): ?>
                    <tr><td colspan="6" class="text-center text-muted py-4">Belum ada booking. Yuk booking slot parkir pertamamu!</td></tr>
                <?php endif; ?>
                </tbody>
            </table>
        </div>
...
document.querySelectorAll('.btn-hapus-booking').forEach(function (btn) {
    btn.addEventListener('click', function (e) {
        ...
        var pesan = 'Hapus booking untuk plat "' + plat + '"?\nTindakan ini tidak bisa dibatalkan.';
        if (status === 'menunggu' || status === 'dikonfirmasi') {
            pesan = 'Booking ini masih aktif (' + status + ').\n' + pesan;
        }
        if (!confirm(pesan)) {
            e.preventDefault();
        }
